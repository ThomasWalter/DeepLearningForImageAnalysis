...
\frame{
  ...
  \begin{itemize}
  \item The neuron computed a linear combination of the inputs
  \item The \alert{bias} $\bias$ is a variable linked to the neuron. It can be interpreted as defining a threshold on the sum
...
\frame{
  ...
  \begin{itemize}
  \item The initial idea behind the activation function is that is works somehow as a gate
...
\frame{
\frametitle{Artificial neuron compact representation}
...
\begin{figure}
\includegraphics[height=6cm]{../graphics/ann_5}
\end{figure}
...
\[
g(b+ \sum\limits_{i=1}^p w_ix_i) = g(b + \W^T\x)
\]
...
\frame{
\frametitle{Artificial Neural Network (ANN)}


\begin{figure}
  \includegraphics[height=4cm]{../graphics/network.png}
\end{figure}
...
\begin{block}{Layers}
  \begin{itemize}
  \item Neurons are typically arranged in \alert{layers}
  \item The first and last layers are respectively called the input and output layers
  \item Any intermediate layers are called \alert{hidden layers}
  \end{itemize}
\end{block}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\frame{
\frametitle{Feed-foward artificial Neural Network (ANN)}
...
\begin{block}{}
  Let $f$ be a continuous real-valued function of $[0,1]^p$ ($p \in \N^*$) and $\epsilon$ a strictly positive real. Let $\act$ be a non-constant, increasing, bounded real function.
...
A first version of this theorem, using sigmoid activation functions, was proposed by \cite{cybenko_approximations_1989}. The version above was demonstrated by \cite{hornik_approximation_1991}.
