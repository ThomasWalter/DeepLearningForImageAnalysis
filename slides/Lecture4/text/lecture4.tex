...
\begin{columns}
  ...
  \begin{column}{.6\textwidth}
    ...
    \item Second solution: regression (macula position)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Transfer learning definition}

\begin{block}{}
  Suppose that thanks to a training set $(X_0, \mathcal{Y_0})$ a model $f_{\theta_0}$ has been learnt.

  Transfer learning consists in learning another model $f_{\theta}$ from a training set $(X, \mathcal{Y})$ using as starting point $f_{\theta_0}$.
...
\begin{frame}{General procedure}
  ...
  \item Optimize the resulting model
  \end{itemize}
...
The authors devised experiments on the ImageNet database in order to evaluate different transfer learning strategies and improve our of understanding of these methods.
...
\item Two separate issues with transfer learning:
  ...
  \end{itemize}
\item Transfer learning is less efficient when the sets are more dissimilar
