...
\begin{frame}{The role of annotated image databases}
  ...
  Image databases including \emph{annotations} (typically some kind of high level information) are essential to the development of machine learning methods for image analysis.
...
\begin{frame}{Loss function for classification: cross-entropy}
  ...
  \begin{alertblock}{}
    To avoid numerical problems, each $\hat{\y}_i$ must be strictly positive. Therefore when used in a NN a softmax operator has to be preceded by a convenient operator, such as a sigmoid or a softmax.
  \end{alertblock}
...
\frame{
  ...
  \begin{itemize}
  \item Convolutional networks often contain subsampling steps. The most popular way of doing this today is by using \emph{max pooling} layers with stride 2.
