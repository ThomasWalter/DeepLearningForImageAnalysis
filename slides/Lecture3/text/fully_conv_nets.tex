...
\begin{block}{Definition: image}
  An 2-dimensional image $I$ of size $p \times q$ ($p, q \in \N^*$) is a function:
\[
...
\begin{block}{Definition: image-to-image neural network}
  An image-to-image NN $F$ is a NN that transforms an image into an image of same size\footnote{In some applications the output size is different from the input size, but for the sake of simplicity we will not consider this case here}:
  \vspace{-1em}
  \begin{align*}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\frame{
\frametitle{Receptive field of the plain CNN}
...
\frame{
\frametitle{Notes on  the U-Net architecture \cite{ronneberger_u-net:_2015}}
...
\item Loss used in the original publication: cross entropy with a weight map $w$ to favor some pixels:
  \[
  L(\param) = \sum\limits_{M \in D} w(M)ln(\hat{y}_{l(M)}(M))
  \]
\end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\frame{
\frametitle{What cost function for image segmentation?}


\begin{block}{Most commonly used cost functions}

  \begin{itemize}
  \item Mean squared error (MSE): $ \sum\limits_i (\hat{y}_i-y_i)^2 $
  \item Cross-entropy: $ -\sum\limits_i y_i\ln(\hat{y}_i) + (1-y_i)\ln(1-\hat{y}_i)$
  \end{itemize}
...
\begin{block}{Dice similarities}
\[
D(\y, \hat{\y}) = \frac{2\sum_i y_i\hat{y}_i}{\sum_i y_i + \sum_i \hat{y}_i}
\]
\end{block}

\begin{block}{Jaccard similarities}
  \[
...
\begin{frame}{Conclusion on loss functions}
  ...
  \item Working on specific losses enforcing structure might be an interesting route...
  \end{itemize}
\end{frame}
...
\frame{
  ...
  \begin {itemize}
  \item Progress in image segmentation since 2012 has been enourmous
