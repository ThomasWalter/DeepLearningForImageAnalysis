...
\item Intractability: the true posterior density $p_{\theta}(z|\x)$ cannot be calculated in polynomial time%= p_{\theta}(\x|z)p_{\theta}(z)/p_{\theta}(\x)$ is intractable.
...
\begin{figure}
\includegraphics[width=.31\columnwidth]{../graphics/ae}
\includegraphics[width=.31\columnwidth]{../graphics/kl}
\includegraphics[width=.31\columnwidth]{../graphics/vae}
\caption{\textbf{Left}: AE, \textbf{Center}: Only KL in VAC, \textbf{Right}: VAE}
\end{figure}
\source{https://towardsdatascience.com/intuitively-understanding-variational-autoencoders}
\end{frame}
...
\textit{``The coolest idea in machine learning in the last 20 years''} - Yann LeCun.
...
\begin{figure}
\includegraphics[width=.99\columnwidth]{../graphics/FACEAGING}
...
\alert{Note:} the discriminator takes as input both images, $(x_i, y_i)$.
...
%% Iterative Gradient Sign Method \cite{kurakin2016adversarial}
