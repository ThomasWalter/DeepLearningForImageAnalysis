...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Major prizes}

  \begin{itemize}
  \item Turing prize 2019: Yann Le Cun, Yoshua Bengio, Geoffrey Hinton
  \item Nobez prize 2024: Geoffrey Hinton, John Hopfield
  \end{itemize}
\end{frame}
...
\begin{itemize}
	\item Machine Learning aims at predicting some output $y$ from an input  $x$:
	\begin{equation}
	y = f(x)
	\end{equation}
...
\begin{itemize}
	\item We choose the following polynomial model:
	\begin{eqnarray}
...
\begin{frame}{Generative adversarial networks \cite{goodfellow_generative_2014}}
  ...
  \begin{block}{Value function}
    $V(G,D) = \mathbb{E}_{p_x}(log(D(x))) + \mathbb{E}_{p_z(z)}(log(1 - D(G(z))))$
  \end{block}
