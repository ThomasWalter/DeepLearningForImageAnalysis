...
\begin{columns}
  ...
  \begin{column}{.5\textwidth}
  \begin{itemize}
  \item Matrices $W^Q$, $W^V$ and $W^V$ are learnable.
  \item $d_K$ is the length of $K'$.
  \end{itemize}
...
\end{columns}
  ...
  \begin{block}{Dot-product attention}
    \[Att(Q,K,V) = softmax\left(\frac{QW^QKW^V}{\sqrt{d_K}}\right) VW^V \]
    \[Att(Q,K,V) = softmax\left(\frac{Q'K'}{\sqrt{d_K}}\right) V' \]
  \end{block}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Dot-product attention}
