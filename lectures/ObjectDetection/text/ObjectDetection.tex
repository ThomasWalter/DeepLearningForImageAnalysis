...
% \end{figure}
% \begin{itemize}
%    \item Ground-truth bounding box is defined by center $(x,y)$ and dimensions $(w,h)$: $G=(x,y,w,h)$ 
%    \item Predicted bounding box: $P=(\hat{x},\hat{y},\hat{w}. \hat{h})$.
%    \item We wish to learn a transformation $t$ that transforms the predicted bounding box $P=(\hat{x},\hat{y},\hat{w}. \hat{h})$ into the ground truth bounding box $G=(x,y,w,h)$. 
% \end{itemize}
% \end{frame}
...
\end{figure}
\begin{itemize}
   \item Ground-truth bounding box is defined by center $(G_x,G_y)$ and dimensions $(G_w,G_h)$: $G=(G_x,G_y,G_w,G_h)$ 
   \item Predicted bounding box: $P=(P_x,P_y,P_w,P_h)$.
   \item We wish to learn a transformation $t$ that transforms the predicted bounding box $P$ into the ground truth bounding box $G$. 
\end{itemize}
\end{frame}
...
%    Regression targets: 
%    \begin{align}
%    t_x &= \frac{\hat{x} - x}{\hat{w}}, &
%    t_y &= \frac{\hat{y} - y}{\hat{h}}, \nonumber \\
%    t_w &= \log \frac{w}{\hat{w}}, &
%    t_h &= \log \frac{h}{\hat{h}},
%    \end{align}
%    %\item This transformation has four parameters, that are predicted from the $P$-crop of the input image feature map. 
% %\end{itemize}
% \end{frame}
...
\begin{frame}{R-CNN: bounding box regression}
\begin{itemize}
   \item $t_x,t_y$ are normalized by the width and height, respectively. This makes the regression scale-invariant, i.e. the network does not need to learn different ranges for small or large boxes.
   \item $t_w,t_h$ are normalized and log-transformed. This leads to symmetric values for the loss (a box twice as big leads to the same loss as a box twice as small, compared to the ground truth). 
   \item At inference, we predict $(\hat{t}_x, \hat{t}_y, \hat{t}_w, \hat{t}_h)$. 
   \item From there, we obtain the refined box: 
   \begin{align}
   \hat{x} &= P_x + \hat{t}_x \cdot P_w, &
   \hat{y} &= P_y + \hat{t}_y \cdot P_h, \nonumber \\
   \hat{w} &= P_w \cdot e^{\hat{t}_w}, &
   \hat{h} &= P_h \cdot e^{\hat{t}_h}.
   \end{align}
\end{itemize}
\end{frame}
...
\item Training in an alternating scheme.
\item Today: detection networks are usually trained end-to-end.
\end{itemize}
\end{frame}
...
%   \includegraphics[height=0.4\textheight]{../graphics/YOLO_workflow.pdf}
   \caption{YOLO: the image is partitioned by an $S \times S$ grid.}
\end{figure}
\begin{itemize}
   \item First, the image is divided into a $S \times S$ grid of cells. 
...
\begin{itemize}
   ...
   \end{itemize}
   \item During training, the confidence score of a predicted bounding box $\hat{B}_i$ is defined as:
   \begin{equation}\nonumber
      Conf_i = P(Obj) IOU(\hat{B}_i, B_i)
   \end{equation}
   where $B_i$ is the ground truth bounding box and $P(Obj)$ the probability that $\hat{B}_i$ contains an object (during training $P(Obj)\in\{0,1\}$)
...
\begin{itemize}
   ...
   \item Each grid cell can contain only $B$ bounding boxes (problems in crowded environments). 
...
\begin{itemize}
   ...
   \item Improvements of YOLO v.2: 
   \begin{itemize}
      \item Better backbone (deeper architecture, batch normalization, ... )
      \item Improvement of box predictions using anchor boxes (anchors are chosen by k-means on the ground truth boxes)
      \item Multi-scale training (by data augmentation)
      \item Combined detection training (on COCO) with classification training (on ImageNet)
      \item $\ldots$
   \end{itemize}
\end{itemize}
\end{frame}
...
\begin{frame}{YOLO v3}
\begin{itemize}
      \item Better backbone (Darknet-53)
      \item Multi-scale prediction (prediction at 3 scales)
      \item More bounding boxes per cell (each grid cell predicts 3 anchor boxes at each scale)
      \item New classification formulation (independent classifications, no softmax) 
      \item $\ldots$
\end{itemize}
\end{frame}
...
%    \item Neck: aggregation and refinement of backbone features to enhance the representation (e.g. multiscale aspects)
%    \item Head: predictions (localization, class, objectness, $\ldots$)
% \end{itemize}
% \end{frame}

\begin{frame}{Most recent version: YOLO v.12}
\begin{figure}[htb]
   \centering
   \includegraphics[width=.7\textwidth]{../graphics/Challenge.jpg}
   \caption{Results of a mitosis detection challenge (YOLO v12: 2nd place)}
\end{figure}
\begin{itemize}
   \item Extensive use of attention mechanisms, identifying regions of importance
   \item Very efficient implementation
   \item Still preprint \cite{Tian2025}
\end{itemize}
\end{frame}
