...
\frame<beamer>{
  ...
  \begin{block}{What is the size of the receptive field of the neurons in the last layer?}
    ...
    \item[B/] $1 + 2 \times n$
    \item[C/] $1 + 2 \times (n+1)$
    \item[D/] It depends on the input image
    \end{itemize}
...
This version works slightly better than the first.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Proposition de stage en collaboration avec L'Oréal}

  \begin{block}{Histologie digitale: évaluation de la pertincence d’approches basées sur l’apprentissage profond pour la segmentation des différentes couches cutanées sur des coupes histologiques provenant d'un modèle de peau reconstruite \textit{in vitro}}

\begin{itemize}
\item Développement d'un réseau de segmentation, capable de généraliser suffisamment bien pour couvrir les variations de coloration et de conditions de culture
\item Etude sur la précision de la vérité-terrain
\item Prise en compte d'information structurelle a priori
\end{itemize}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth]{a_effacer1}
\end{figure}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth]{a_effacer2}
\end{figure}
