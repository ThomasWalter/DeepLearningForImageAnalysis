...
\frame{
  ...
  \item Familiarize yourself with the problem and the data
    \begin{itemize}
    \item Choose a test metric
    \end{itemize}
  \item Cast the task at hand into a machine learning problem
  \item Build and prepare your data set
    \begin{itemize}
    \item Annotations
    \item Representativeness
    \item Define train, validation and test subsets (avoid data leaking)
    \end{itemize}
  \item Build an architecture and train it or choose a pre-trained model and use transfer learning

\begin{itemize}[<+->]
  \item Analyze the results on the validation data (\alert{look} at the images!)
  \item Increase the power of your model until you reach over-fitting. Then, regularize.
  \item Log all your experiments.
\end{itemize}
...
\frame{
  ...
  Cast your problem into a convenient representation:
    ...
    \begin {itemize}[<+->]
    \item Choose the right representation for your data.
      \begin{itemize}
      \item Resolution, channels
      \item Regression or classification?
      \item Output format?
      \end{itemize}
    \item Define a validation procedure
    \end{itemize}
...
\begin{columns}
  ...
  \begin{column}{.6\textwidth}
    ...
    \pause
    \begin{itemize}
    \item First solution: global classification (is the macula visible?)
    \item Second solution: regression (macula center coordinates)
    \item Third solution: macula segmentation
      \end{itemize}
  \end{column}
\end{columns}

\source{images from the OPHDIAT database}
\end{frame}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth]{alais_ex1}
\end{figure}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth]{alais_ex2}
\end{figure}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth]{alais_ex3}
\end{figure}
...
\frame{
  ...
  \item Deep learning models tend to be robust with respect to ground-truth errors
    \begin{itemize}
    \item if they are not too frequent
    \item and if they are not systematic.
    \end{itemize}
...
\frame{
  ...
  \item AugMix, CutMix, etc. (cf. lesson on optimization)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Unsupervised domain adaptation}

  \begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{ganin_images}
    \source{\cite{ganin_unsupervised_2015}}
  \end{figure}

  The labels of the source domain are known, but not those of the source domain. How to proceed ?

\end{frame}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Example method~\cite{ganin_unsupervised_2015}}

  \begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{ganin_method}
  \end{figure}
...
\begin{frame}{Results}
    ...
    \begin{figure}[ht]
    \includegraphics[width=0.83\textwidth, right]{ganin_images}
  \end{figure}

  \scriptsize
\begin{table}[]
\begin{tabular}{lr||c|c|c|c}
  \multirow{2}{*}{Method} & Source  & MNIST   & SYN NUMBERS &  SVHN  & SYN SIGNS \\
                          & Target &  MNIST-M &  SVHN       &  MNIST & GTSRB      \\ \hline
\multicolumn{2}{l||}{Source only} & 0.575 & 0.867 & 0.592 & 0.740 \\
\multicolumn{2}{l||}{Proposed approach} & 0.815 & 0.905 & 0.711 & 0.887 \\
\multicolumn{2}{l||}{Train on target} & 0.989 & 0.924 & 0.995 & 0.998
\end{tabular}
\end{table}
\source{\cite{ganin_unsupervised_2015}}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Related subjects}

  \begin{itemize}
  \item Foundation models
\pause
\begin{block}{Definition~\cite{bommasani_opportunities_2022}}
  ``A \alert{foundation model} is any model that is trained on broad data (generally using self-supervision at scale) that can be adapted (e.g., fine-tuned) to a wide range of downstream tasks.''
\end{block}
\pause
...
%%   \begin{itemize}
%%   \item Some large language model families: GPT, LlaMA, Mistral
%%   \item Computer vision model families: Segment Anything Model, DINO
%%   \end{itemize}
%% \end{frame}
...
%%       \item Non supervised methods, including autoencoders and generative adversarial networks
