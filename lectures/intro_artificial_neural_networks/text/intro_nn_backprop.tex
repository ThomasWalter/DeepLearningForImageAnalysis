...
\date{Mines Paris\\
  ...
  Center for Statistics and Images
}
...
\frame{
  ...
  \begin{block}{Remarks}
    ...
    \item[-] Its derivative is undefined at $0$, equal to $0$ everywhere else
...
\frame{
  ...
  \begin{block}{}
    ...
    corresponds to a hyperplane $H$ (provided $\mathbf{w}$ is not equal to $\mathbf{0}$). For a given point
    $\x = \{x_1, \ldots, x_p\}$,
...
\frame{
  ...
  \begin{alertblock}{}
    In the remainder of this course, except when otherwise specified, all NNs will be feed-forward. Indeed, this is the preferred type of NN for image processing.
  \end{alertblock}
...
\frame{
  ...
  \item  If a FC layer contains $r$ neurons, and the previous layer $q$, then its weights are a 2-dimensional array (a matrix) of size $q \times r$
  \end{itemize}
...
\begin{frame}{A composition of differentiable functions}
  ...
  \begin{itemize}
  \item The functions composing an artificial neural network are differentiable (almost everywhere). Therefore, an ANN is differentiable (almost everywhere).
  \item In fact, any continuous function on a closed bounded domain can be approximated within any error margin by an artificial neural network.
...
\begin{frame}{Introduction}
  ...
  \begin{itemize}
  \item How can the parameters $\param = (\W_i, \bias_i)$  be set?
...
\begin{frame}{Supervised learning problem}
  ...
  depending on a set of parameters $\param$. Our objective is to \textcolor{blue}{find} the value $\param ^{\ast}$ of $\param$
  that minimizes:
...
\begin{frame}{Gradient descent in the scalar case}
  ...
  \begin{columns}
    ...
    \begin{column}{.5\textwidth}
      ...
      $\lr$ is called the \alert{learning rate}.
    \end{column}
  \end{columns}
...
\begin{frame}{Stochastic gradient descent}
  ...
  \item
    ...
    \[
    \loss_{stoch}(\param_t) = l(y_i, f(\x_i, \param_t))
    \]
  \item
    The gradient $\nabla \loss_{stoch}(\param_t)$ is computed and
  \item Finally the parameters are updated:

    \[
    \param_{t+1} = \param_t - \lr\nabla \loss_{stoch}(\param_t)
    \]
  \item
...
\end{itemize}
  ...
  \begin{block}{}
        ...
        \begin{pmatrix}
          ...
          \vdots  & \ddots & \vdots  \\
          \frac{\partial \mathcal{M}_{p,1}}{\partial \X} & \cdots & \frac{\partial \mathcal{M}_{p,q}}{\partial \X}
        \end{pmatrix}
        $

      \end{block}

  This is an array of size $(p, q, m, n)$.
...
\begin{frame}{Backpropagation through an activation function $g$}
  ...
  \item Therefore: $\pdv{\Y_{i,j}}{\X_{i,j}} = \act'(X_{i,j})$.
  \item Finally:
    \[
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Network parameters initialisation}
...
\begin{frame}{Network parameters initialization}
  ...
  \begin{itemize}
   \item \cite{lecun_efficient_1998}: Lecun et al. suggest drawing the weights from a normal distribution of mean $0$ and variance $1/n$  (where $n$ is the number of inputs of a neuron)
  \item \cite{he_delving_2015}: in the case of ReLU activations, He et al. recommend a $2/n$ variance
  \end{itemize}
