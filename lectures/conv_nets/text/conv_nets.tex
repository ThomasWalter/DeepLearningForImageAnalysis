...
\begin{frame}{Quizz}
  ...
  \begin{block}{The first course session was:}
    \begin{itemize}
    \item[A/] An unnecessary repetition
    \item[B/] A welcome reminder
    \item[C/] Too fast / too dense
    \end{itemize}
...
\frame<beamer>{
  ...
  \begin{columns}
    ...
    \begin{column}{0.45\textwidth}
      ...
      \begin{block}{The best solution would be to:}
    \begin{itemize}
    \item[A/] Integrate along the horizontal or vertical axis, in order to get rid of a dimension
    \item[B/] Apply the network to each row or column separately
    \item[C/] Transform the image into a vector, by rearranging the pixels
    \end{itemize}
...
\frame{
  ...
  \item do not take advantage of the local structure of images
...
\begin{columns}
  ...
  \begin{block}{How many parameters do we have in layer 2?}
    \begin{itemize}
    \item[A/] $d^2 \times d^1 \times s$
    \item[B/] $d^2 \times (d^1 \times s + 1)$
    \item[C/] $(d^2 + 1) \times (d^1 \times s + 1)$
    \end{itemize}
  \end{block}
  \end{column}
\end{columns}
