...
\begin{frame}{Classical image processing approach}
  ...
  \item[\boldmath$\ominus$] If objects are difficult to model, machine learning methods can bring a solution.
  \end{itemize}
...
\frame{
  ...
  \item[\boldmath$\ominus$] An expert is required to define those features - and this can be a long process
...
\frame{
  ...
  \begin{figure}
    \includegraphics[height=3.5cm]{mini_reseau3_bis}
  \end{figure}
...
\frame{
  ...
  \begin{itemize}
  \item NN solely composed of fully-connected layers are almost never used for image analysis (however, \alert{transformers}~\cite{dosovitskiy_image_2021} have brought new ways to use fully-connected layers).
